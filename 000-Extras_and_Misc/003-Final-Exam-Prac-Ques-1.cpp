...
public:
    ...
    Box& operator+=(const Gem& gem) {
        std::shared_ptr<Gem> newGem(new Gem(gem));
        m_gems.push_back(newGem);
        return *this;
    };

    Box& operator-=(const string& name) {
        size_t curSize = m_gems.size();
        auto itr = std::remove_if(m_gems.begin(), m_gems.end(), [=](const shared_ptr<Gem>& g) {
            return g->name == name;
        });
        m_gems.resize(itr - m_gems.begin());
    ...
    void process() {
        transform(m_gems.begin(), m_gems.end(), m_gems.begin(), [](shared_ptr<Gem>& gem) {
            if (!gem->polished) {
                gem->polished = true;
                gem->shine += 20;
            }
